...
import type { RequestContext, MeetingRoomConfig } from '../types/index.js';

/**
 * 从请求头获取上下文
 */
function getContextFromRequest(req: Request): RequestContext {
  const appToken = req.headers['x-app-token'] as string;
  const personalBaseToken = req.headers['x-personal-base-token'] as string;
 ...
 * 发送成功响应
 */
function sendSuccess<T>(res: Response, data: T, message = 'success') {
  res.json({
    code: 0,
    message,
    data,
 ...
 * 发送错误响应
 */
function sendError(res: Response, message: string, code = 500, statusCode = 500) {
...
export const BaseController = {
   ...
   */
  async getFieldList(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
   ...
   */
  async getRecordList(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
      const { tableId } = req.params;
      const { viewId, filter } = req.query as { viewId?: string; filter?: string };
   ...
   */
  async getCellValue(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
   ...
   */
  async getAttachmentUrls(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
      const { tableId, recordId } = req.params;
   ...
   */
  async proxyAttachment(req: Request, res: Response) {
    ...
    try {
      ...
      const data = await response.json() as { code: number; msg?: string; data?: { tmp_download_urls?: Array<{ tmp_download_url: string }> } };
...
export const MeetingRoomController = {
   ...
   */
  async loadMeetingData(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
      const config: MeetingRoomConfig = req.body;
  ...
  async checkConflict(req: Request, res: Response) {
    try {
      ...
      const meetingRoomService = new MeetingRoomService(context);
      const result = await meetingRoomService.checkTimeConflict(
        bookingTableId,
        roomName,
        startTime,
        endTime,
   ...
   */
  async quickBook(req: Request, res: Response) {
    try {
      const context = getContextFromRequest(req);
      const {
        bookingTableId,
      ...
      // 先检查时间冲突
      const meetingRoomService = new MeetingRoomService(context);
      const conflictResult = await meetingRoomService.checkTimeConflict(
        bookingTableId,
        roomName,
        startTime,
        endTime,
