...
export class BaseService {
  ...
  async getTableList(): Promise<TableInfo[]> {
    try {
      ...
      return response.data.items.map((table: any) => ({
        id: table.table_id,
        name: table.name,
      }));
    } catch (error) {
   ...
   * 获取表格字段列表
   */
  async getFieldList(tableId: string): Promise<FieldInfo[]> {
    try {
  ...
  async getRecordList(
    ...
    try {
      ...
      for await (const response of await this.client.base.appTableRecord.listWithIterator({
        ...
        if (response?.items) {
          records.push(...response.items.map((record: any) => {
            // 将字段名称转换为字段 ID
            const fieldsWithId: Record<string, any> = {};
   ...
   * 获取单个记录
   */
  async getRecord(tableId: string, recordId: string): Promise<RecordInfo> {
    try {
   ...
   * 获取字段配置
   */
  async getFieldConfig(tableId: string, fieldId: string): Promise<any> {
    try {
      // 获取所有字段列表，然后找到匹配的字段
      const fields = await this.getFieldList(tableId);
  ...
  async createRecord(
    ...
    try {
      ...
      // 检查 SDK 响应中是否有错误码
      if ((response as any).code !== undefined && (response as any).code !== 0) {
        throw new Error((response as any).msg || '创建记录失败');
      }
